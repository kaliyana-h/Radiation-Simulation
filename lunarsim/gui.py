...
from .dosimetry import (assess, assess_composition, assess_spe,
                        ...
                        _gcr_thinwall_calibrated, areal_density_confidence)
...
def _report_text(job, gcr=None, spe=None, thinwall=None) -> str:
    ...
    if gcr is not None:
        ...
        L += [f"- Absorbed dose: {head.annual_mgy:.1f} mGy/yr",
              ...
              f"- **Verdict: {s['verdict']}**"]
        cband = areal_density_confidence(spec.areal_density_gcm2())
        L.append(f"- **Confidence ({cband['label']}):** {cband['message']}")
